...
let Navbar = () => {
  ...
  const getUser = async() =>{
    const { data } = await axios.get("https://devconnect-backend-nno6.onrender.com/api/users/me", {
      headers: {
 ...
 if(localStorage.getItem("devconnect"))
  ...
  let clickLogOut = async() => {
    localStorage.removeItem("devconnect");
      ...
      <nav className="navbar fixed-top navbar-dark bg-dark navbar-expand-sm">
        ...
        {/* Logo and name of App*/}
          <Link to="/" className="navbar-brand">
            <i className="fa fa-code" /> Dev-Connect
          </Link>
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul className="navbar-nav ml-auto">
              {localStorage.getItem("devconnect") ? afterLogin : beforeLogin}
            </ul>
          </div>
        </div>
      </nav>
